...
export class CourseAnswerService {
  httpOptions = {
  ...
  store(course_answer: CourseAnswer): Observable<CourseAnswer> {
    return this.http.post<CourseAnswer>(AppConfig.API_ENDPOINT + 'course_answers', course_answer, this.httpOptions)
      .pipe(
        tap(_ => this.messageService.showMessage('Question updated successfully.')),
        catchError(this.handleError<CourseAnswer>('store Chapter}'))
      )
  }
  ...
  // }
  //
  delete(course_answer: CourseAnswer | number): Observable<CourseAnswer> {
    const id = typeof course_answer === 'number' ? course_answer : course_answer.id;
    return this.http.delete<CourseAnswer>(AppConfig.API_ENDPOINT + 'course_answers/' + id, this.httpOptions)
    .pipe(
      // tap(_ => this.messageService.showMessage('Material deleted successfully.')),
      catchError(this.handleError<CourseAnswer>('delete answer;'))
    );
  };
